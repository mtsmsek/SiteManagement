...
namespace SiteManagement.XUnitTests.Application.Features.Vehicles.CreateVehicle;
...
public class CreateVehicleTest : VehicleMockRepository
{
    ...
    public CreateVehicleTest(VehicleFakeData fakeData, CreateVehicleCommand command, CreateVehicleCommandValidator validator) : base(fakeData)
    {
        ...
        _handler = new(MockRepository.Object, Mapper, residentRepository.Object.MockRepository.Object, BusinessRules, residentVehicleRepository.Object, apartmentBusinessRules);
    }
    ...
    {
        //Arrange
        _command.ApartmentId = ResidentFakeDatas.Under18ResidentId;
        ...
        //Assert
        var response = await Assert.ThrowsAsync<BusinessException>(Action);
        Assert.Equal(VehicleMessages.RuleMessages.AllUsersYoungerThan18, response.Message);

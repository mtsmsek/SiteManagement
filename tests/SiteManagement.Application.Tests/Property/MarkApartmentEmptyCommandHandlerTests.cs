...
/// <summary>
/// Unit tests for <see cref="MarkApartmentEmptyCommandHandler"/>: resolve the
/// owning site, flip an occupied apartment back to empty, and save — or 404 when
/// no site contains the apartment.
/// </summary>
public class MarkApartmentEmptyCommandHandlerTests
{
    private readonly ISiteRepository _siteRepository = Substitute.For<ISiteRepository>();
    private readonly IUnitOfWork _unitOfWork = Substitute.For<IUnitOfWork>();

    [Fact]
    public async Task Handle_MarksTheApartmentEmptyAndSaves()
    {
        // arrange — a site containing an already-occupied apartment
        ...
        // act
        await sut.Handle(new MarkApartmentEmptyCommand(apt.Id), CancellationToken.None);

        // assert
        apt.Status.Should().Be(OccupancyStatus.Empty);
    ...
    public async Task Handle_UnknownApartment_Throws()
    {
        ...
        // act
        var act = () => sut.Handle(new MarkApartmentEmptyCommand(Guid.NewGuid()), CancellationToken.None);

        // assert
        await act.Should().ThrowAsync<EntityNotFoundException>();
    }

    private MarkApartmentEmptyCommandHandler CreateHandler() => new(_siteRepository, _unitOfWork);
}
